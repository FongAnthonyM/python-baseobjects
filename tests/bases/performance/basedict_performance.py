...
class TestBaseDictPerformance(BasePerformanceTestSuite):
    ...
    @pytest.fixture
    def test_user_dict(self) -> UserDict[Any, Any]:
    ...
    def test_creation_performance(self) -> None:
        ...
        print(
            f"\nStandard dict creation: {mean_std:.3f} μs "
        ...
        print(
            f"BaseDict creation: {mean_base:.3f} μs ({percent_user:.3f}% of UserDict creation time, "
            f"{percent_std:.3f}% of standard dict creation time)",
        )
    ...
    def test_get_item_performance(
        ...
        test_user_dict: UserDict[Any, Any],
        ...
        # Print the performance comparison
        print(
            f"\nStandard dict item access: {mean_std:.3f} μs "
            f"({self.call_speed:.3f} is the speed of a simple function call)",
        )
        print(f"UserDict item access: {mean_user:.3f} μs ({mean_user / mean_std:.3f}x standard dict)")
        print(
            f"BaseDict item access: {mean_base:.3f} μs ({percent_user:.3f}% of UserDict item access time, "
            f"{percent_std:.3f}% of standard dict item access time)",
        )
    ...
    def test_set_item_performance(
        ...
        # Print the performance comparison
        print(
            f"\nStandard dict item setting: {mean_std:.3f} μs "
            f"({self.call_speed:.3f} is the speed of a simple function call)",
        )
        print(f"UserDict item setting: {mean_user:.3f} μs ({mean_user / mean_std:.3f}x standard dict)")
        print(
            f"BaseDict item setting: {mean_base:.3f} μs ({percent_user:.3f}% of UserDict item setting time, "
            f"{percent_std:.3f}% of standard dict item setting time)",
        )
        assert percent_user < self.speed_tolerance * 1.5  # Allow some overhead compared to UserDict
        assert percent_std < self.speed_tolerance * 2  # Allow more overhead compared to standard dict
    ...
    def test_delete_item_performance(
        ...
        # Print the performance comparison
        print(
            f"\nStandard dict item deletion: {mean_std:.3f} μs "
            f"({self.call_speed:.3f} is the speed of a simple function call)",
        )
        print(f"UserDict item deletion: {mean_user:.3f} μs ({mean_user / mean_std:.3f}x standard dict)")
        print(
            f"BaseDict item deletion: {mean_base:.3f} μs ({percent_user:.3f}% of UserDict item deletion time, "
            f"{percent_std:.3f}% of standard dict item deletion time)",
        )
        assert percent_user < self.speed_tolerance * 1.5  # Allow some overhead compared to UserDict
        assert percent_std < self.speed_tolerance * 2  # Allow more overhead compared to standard dict
    ...
    def test_iteration_performance(
        ...
        # Print the performance comparison
        print(
            f"\nStandard dict iteration: {mean_std:.3f} μs "
            f"({self.call_speed:.3f} is the speed of a simple function call)",
        )
        print(f"UserDict iteration: {mean_user:.3f} μs ({mean_user / mean_std:.3f}x standard dict)")
        print(
            f"BaseDict iteration: {mean_base:.3f} μs ({percent_user:.3f}% of UserDict iteration time, "
            f"{percent_std:.3f}% of standard dict iteration time)",
        )
        assert percent_user < self.speed_tolerance * 1.5  # Allow some overhead compared to UserDict
        assert percent_std < self.speed_tolerance * 2  # Allow more overhead compared to standard dict
    ...
    def test_copy_performance(
        ...
        print(
            f"BaseDict copy: {mean_base:.3f} μs ({percent_user:.3f}% of UserDict copy time, "
            f"{percent_std:.3f}% of standard dict copy time)",
        )
        assert percent_user < self.speed_tolerance * 2  # Allow more overhead compared to UserDict
        assert percent_std < self.speed_tolerance * 3  # Allow more overhead compared to standard dict
    ...
    def test_deepcopy_performance(
        ...
        # Print the performance comparison
        print(
            f"\nStandard dict deepcopy: {mean_std:.3f} μs "
            f"({self.call_speed:.3f} is the speed of a simple function call)",
        )
        print(f"UserDict deepcopy: {mean_user:.3f} μs ({mean_user / mean_std:.3f}x standard dict)")
        print(
            f"BaseDict deepcopy: {mean_base:.3f} μs ({percent_user:.3f}% of UserDict deepcopy time, "
            f"{percent_std:.3f}% of standard dict deepcopy time)",
        )
        assert percent_user < self.speed_tolerance * 2  # Allow more overhead compared to UserDict
        assert percent_std < self.speed_tolerance * 3  # Allow more overhead compared to standard dict
