...
class TestBaseListPerformance(BasePerformanceTestSuite):
    ...
    @pytest.fixture
    def test_user_list(self) -> UserList[Any]:
    ...
    def test_creation_performance(self) -> None:
        ...
        print(
            f"\nStandard list creation: {mean_std:.3f} μs "
        ...
        print(
            f"BaseList creation: {mean_base:.3f} μs ({percent_user:.3f}% of UserList creation time, "
            f"{percent_std:.3f}% of standard list creation time)",
        )
    ...
    def test_get_item_performance(
        ...
        # Print the performance comparison
        print(
            f"\nStandard list item access: {mean_std:.3f} μs "
            f"({self.call_speed:.3f} is the speed of a simple function call)",
        )
        print(f"UserList item access: {mean_user:.3f} μs ({mean_user / mean_std:.3f}x standard list)")
        print(
            f"BaseList item access: {mean_base:.3f} μs ({percent_user:.3f}% of UserList item access time, "
            f"{percent_std:.3f}% of standard list item access time)",
        )
    ...
    def test_set_item_performance(
        ...
        # Print the performance comparison
        print(
            f"\nStandard list item setting: {mean_std:.3f} μs "
            f"({self.call_speed:.3f} is the speed of a simple function call)",
        )
        print(f"UserList item setting: {mean_user:.3f} μs ({mean_user / mean_std:.3f}x standard list)")
        print(
            f"BaseList item setting: {mean_base:.3f} μs ({percent_user:.3f}% of UserList item setting time, "
            f"{percent_std:.3f}% of standard list item setting time)",
        )
        assert percent_user < self.speed_tolerance * 1.5  # Allow some overhead compared to UserList
        assert percent_std < self.speed_tolerance * 2  # Allow more overhead compared to standard list
    ...
    def test_append_performance(
        ...
        print(
            f"BaseList append: {mean_base:.3f} μs ({percent_user:.3f}% of UserList append time, "
            f"{percent_std:.3f}% of standard list append time)",
        )
        assert percent_user < self.speed_tolerance * 1.5  # Allow some overhead compared to UserList
        assert percent_std < self.speed_tolerance * 2  # Allow more overhead compared to standard list
    ...
    def test_extend_performance(
        ...
        print(
            f"BaseList extend: {mean_base:.3f} μs ({percent_user:.3f}% of UserList extend time, "
            f"{percent_std:.3f}% of standard list extend time)",
        )
        assert percent_user < self.speed_tolerance * 1.5  # Allow some overhead compared to UserList
        assert percent_std < self.speed_tolerance * 2  # Allow more overhead compared to standard list
    ...
    def test_insert_performance(
        ...
        print(
            f"BaseList insert: {mean_base:.3f} μs ({percent_user:.3f}% of UserList insert time, "
            f"{percent_std:.3f}% of standard list insert time)",
        )
        assert percent_user < self.speed_tolerance * 1.5  # Allow some overhead compared to UserList
        assert percent_std < self.speed_tolerance * 2  # Allow more overhead compared to standard list
    ...
    def test_iteration_performance(
        ...
        # Print the performance comparison
        print(
            f"\nStandard list iteration: {mean_std:.3f} μs "
            f"({self.call_speed:.3f} is the speed of a simple function call)",
        )
        print(f"UserList iteration: {mean_user:.3f} μs ({mean_user / mean_std:.3f}x standard list)")
        print(
            f"BaseList iteration: {mean_base:.3f} μs ({percent_user:.3f}% of UserList iteration time, "
            f"{percent_std:.3f}% of standard list iteration time)",
        )
        assert percent_user < self.speed_tolerance * 1.5  # Allow some overhead compared to UserList
        assert percent_std < self.speed_tolerance * 2  # Allow more overhead compared to standard list
    ...
    def test_copy_performance(
        ...
        print(
            f"BaseList copy: {mean_base:.3f} μs ({percent_user:.3f}% of UserList copy time, "
            f"{percent_std:.3f}% of standard list copy time)",
        )
        assert percent_user < self.speed_tolerance * 2  # Allow more overhead compared to UserList
        assert percent_std < self.speed_tolerance * 3  # Allow more overhead compared to standard list
    ...
    def test_deepcopy_performance(
        ...
        # Print the performance comparison
        print(
            f"\nStandard list deepcopy: {mean_std:.3f} μs "
            f"({self.call_speed:.3f} is the speed of a simple function call)",
        )
        print(f"UserList deepcopy: {mean_user:.3f} μs ({mean_user / mean_std:.3f}x standard list)")
        print(
            f"BaseList deepcopy: {mean_base:.3f} μs ({percent_user:.3f}% of UserList deepcopy time, "
            f"{percent_std:.3f}% of standard list deepcopy time)",
        )
        assert percent_user < self.speed_tolerance * 2  # Allow more overhead compared to UserList
        assert percent_std < self.speed_tolerance * 3  # Allow more overhead compared to standard list
